...
${"┗" + "━".repeat(if (missing % 2 == 0) missing / 2 else (missing / 2) + 1) + version + "━".repeat(missing / 2) + "┛"}
    ...
    val importInvoiceService = ImportInvoiceDbService(database)
    val importFileManager = ImportFileManager(dataPath)
    val importService = ImportService(importFileManager, importDocumentDb, importPageDb, importBlockDb, importLineDb, importWordDb, importInvoiceService)
